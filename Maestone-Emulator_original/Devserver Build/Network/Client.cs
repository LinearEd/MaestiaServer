    ...
    public class Client
    {
        ...
        private void ReceivePacket(IAsyncResult asyncResult)
        {
            try
            {
                ...
                Log.WriteInfo($"[{_serverListener.HandlerType}] Received {recvLength} bytes.");
                ...
                while (recvLength > processedLength)
                {
                    ...
                    Log.WriteInfo($"[{_serverListener.HandlerType}] Processing packet of length {packetLength}.");
            ...
            {
                Log.WriteError($"[{_serverListener.HandlerType}] Error receiving packet: {ex.Message}");
            }
            finally
            {
                _clientStream.BeginRead(_clientBuffer, 0, _clientBuffer.Length, ReceivePacket, null);
